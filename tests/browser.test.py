...
try:
 with sync_playwright() as p:
    ...
    record('Mobile: default quality is balanced',page.evaluate("__SD.settings.quality==='balanced'&&__SD.settings.mobile"))
    ...
    record('Mobile: landscape resize keeps balanced profile',page.evaluate('__SD.settings.mobile&&__SD.settings.quality==="balanced"&&__SD.renderer.width>__SD.renderer.height'))
    ...
    record('Landscape cold start: selects balanced profile despite width > 760px',page.evaluate('__SD.settings.quality==="balanced"&&__SD.settings.mobile'))
    page.close();ctx.close()
    ...
    record('Desktop: audio is initially off',page.evaluate("document.getElementById('audio-toggle').dataset.audioState==='off'&&document.getElementById('audio-toggle').getAttribute('aria-pressed')==='false'"))
    page.click('#audio-toggle');page.wait_for_function("document.getElementById('audio-toggle').dataset.audioState==='playing'",timeout=10000)
    record('Desktop: audio starts after an explicit click',page.evaluate("document.getElementById('audio-toggle').getAttribute('aria-pressed')==='true'"))
    page.click('#audio-toggle');page.wait_for_function("document.getElementById('audio-toggle').dataset.audioState==='off'",timeout=10000)
    record('Desktop: audio can be switched off again',page.evaluate("document.getElementById('audio-toggle').getAttribute('aria-pressed')==='false'"))
